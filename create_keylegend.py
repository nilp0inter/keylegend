...
def create_keylegend(title, action, icon, output_file, title_font_size, action_font_size, title_font_family, action_font_family, background_color, title_color, action_color):
    ...
    if icon:
        ...
        d.append(draw.Text(icon_glyph, action_font_size, 0, (title_font_size + 2 * MARGIN) / 2 - action_font_size / 4, text_anchor='middle', font_family='Font Awesome 5 Free', font_weight='900', fill=action_color))
    elif '$' not in action:
...
def main():
    ...
    parser.add_argument('--action-font-size', type=int, default=22, help='Font size for the action text or icon. Default is 22.')
    ...
    if args.icon and args.action:
        print("Error: Please provide either action text or an icon, not both.")
        return
